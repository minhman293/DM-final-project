# predict_lstm.py — Generate submission using the BEST validation model
#
# Phase 1 change: load 'lstm_weights_best.pth' instead of 'lstm_weights.pth'.
# The best-validation checkpoint is saved by train_lstm.py.
...
def main():
    ...
    # ── 1. Compute region baselines from training data ────────────────────────
    ...
    test_weekly = add_phase2_features(test_weekly)
    ...
    # ── 3. Load encoders, scaler, and best model ──────────────────────────────
    ...
    logger.info("Loading BEST LSTM model (lowest val MAE)...")
    model = DisasterLSTM(
    ...
    best_weights_path = MODEL_DIR / "lstm_weights_best.pth"
    model.load_state_dict(torch.load(best_weights_path, map_location=device))
    model.eval()
    logger.info(f"Loaded weights from: {best_weights_path}")

    # ── 4. Generate predictions per region ────────────────────────────────────
    ...
    # ── 5. Format submission and apply threshold squeezer ─────────────────────
    ...
    # Threshold squeezer: small predictions snap to 0
    submission[pred_cols] = submission[pred_cols].where(
        submission[pred_cols] >= THRESHOLD_SQUEEZER, 0.0
    )

    submission[pred_cols] = submission[pred_cols].round(4)

    out_path = SUBMISSION_DIR / "submission_lstm_v4.csv"
    ...
    for col in pred_cols:
        logger.info(
            f"  {col}: mean={submission[col].mean():.3f}, "
            f"std={submission[col].std():.3f}, "
            f"zeros={(submission[col] == 0).sum()}, "
