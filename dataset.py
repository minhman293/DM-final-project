# dataset.py — PyTorch Dataset for LSTM with validation support
#
# Phase 1 changes:
# - Support filtering by region_id list (for train/val split)
# - Better sliding window: also store the source region_id for debugging
...
class DisasterSequenceDataset(Dataset):
    """
    PyTorch Dataset for LSTM training and inference.

    Train mode (is_train=True):
        Uses sliding window over each region's weekly series to generate
        many (input_sequence, target_sequence) pairs.

    Test mode (is_train=False):
        Each region contributes exactly 1 sample — the last SEQ_LEN weeks.
    """
    ...
    def __init__(
        ...
        """
        Args:
            weekly_df       : DataFrame with weekly features per region
            region_encoder  : dict mapping region_id -> int index
            scaler          : fitted sklearn StandardScaler
            is_train        : True for training samples, False for inference
            region_filter   : optional list of region_ids to keep.
        ...
        if region_filter is not None:
            weekly_df = weekly_df[weekly_df["region_id"].isin(region_filter)]
        ...
        for region_id, group in weekly_df.groupby("region_id"):
            ...
            if region_id not in region_encoder:
                # Skip regions not seen during encoder fit (shouldn't happen)
                continue
            ...
            if is_train:
                ...
                # Sliding window over the region's full history
                for i in range(n_weeks - SEQ_LEN - PRED_LEN + 1):
                    ...
                    # Summer flag: prediction window starts in months 5-10
                    target_month = months[i + SEQ_LEN]
                    is_summer = 1.0 if target_month in [5, 6, 7, 8, 9, 10] else 0.0
                    self.summers.append(is_summer)
            else:
                # For test set: exactly 13 weeks per region, use the last SEQ_LEN
